...
SplashScreen::SplashScreen(runtime::NativeWindow* window,
                           ...
                           ss_info, const std::string& app_path) :
                           is_active_(false) {
...
void SplashScreen::SetBackground(const wgt::parse::SplashScreenData&
                                       splash_data,
                                      Evas_Object* parent,
  ...
  if (!splash_data.background_image[0].empty()) {
    elm_bg_file_set(background_, (app_path +
                    splash_data.background_image[0]).c_str(), NULL);
    ...
    elm_bg_color_set(background_, splash_data.color.red,
                     splash_data.color.green,
                     splash_data.color.blue);
  }
  evas_object_show(background_);
}

void SplashScreen::SetImage(const wgt::parse::SplashScreenData&
                              splash_data,
                             Evas_Object* parent,
                             const std::pair<int,int>& pair,
                             const std::string& app_path) {
